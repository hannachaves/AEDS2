class Is {
    ...
    public boolean vogal(String line, int i) {
        boolean resp = true;
    ...
    public boolean consoante(String line) {
        return consoante(line, 0);
    }

    public boolean consoante(String line, int i) {
        boolean resp = true;
    ...
    public boolean inteiro(String line) {
        return inteiro(line, 0);
    }

    public boolean inteiro(String line, int i) {
        boolean resp = true;
    ...
    public boolean isReal(String line) {
        return vogal(line, 0, 0);
    }

    public boolean isReal(String line, int i, int j) {
        boolean resp = false;

        if(i < line.length()) {
            if(isInteiro(line.charAt(i))) {
                resp = isReal(line, i + 1, j);
            } 
            else if((line.charAt(i) == '.') || (line.charAt(i) == ',')) {
                resp = isReal(line, i + 1, j + 1);
            }
            else {
                resp = false;
            }
